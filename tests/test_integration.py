...
def nsbox_run_code(code: str) -> tuple[str, int]:
    body = {"args": ["-c", code]}
    return nsbox_request(body)


def nsbox_request(content: dict) -> tuple[str, int]:
...
class IntegrationTests(unittest.TestCase):
    def test_memory_limit_separate_per_process(self):
        ...
        See https://github.com/python-discord/nsbox/issues/83
        """
        with run_gunicorn():
            ...
            with Pool(processes) as p:
                results = p.map(nsbox_run_code, args)
    ...
    def test_files_send_receive(self):
        ...
        with run_gunicorn():
            ...
            response, status = nsbox_request(request)
