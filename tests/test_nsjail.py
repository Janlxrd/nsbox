...
from nsbox.filesystem import Size
...
class NsJailTests(unittest.TestCase):
    def setUp(self):
        ...
        self.logger = logging.getLogger("nsbox.nsjail")
    ...
    def test_read_only_file_system(self):
        for path in ("/", "/etc", "/lib", "/lib64", "/nsbox", "/usr"):
    ...
    @unittest.mock.patch("nsbox.nsjail.DEBUG", new=False)
    def test_log_parser(self):
        ...
        self.assertIn("DEBUG:nsbox.nsjail:This is a debug message.", log.output)
        self.assertIn("ERROR:nsbox.nsjail:Couldn't parse cmdline options", log.output)
        self.assertIn("ERROR:nsbox.nsjail:No command-line provided", log.output)
        self.assertIn("WARNING:nsbox.nsjail:Failed to parse log line 'Invalid Line'", log.output)
        self.assertIn("WARNING:nsbox.nsjail:This is a warning!", log.output)
        self.assertIn(
            "INFO:nsbox.nsjail:pid=20 ([STANDALONE MODE]) exited with status: 2, (PIDs left: 0)",
            log.output,
        )
    ...
    def test_run_args(self):
        ...
        for args, expected in cases:
            with self.subTest(args=args):
                result = self.nsjail.python3(run_args=args)
...
class NsJailArgsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config_path = shutil.copy2("./config/nsbox.cfg", self._temp_dir.name)
...
class NsJailCgroupTests(unittest.TestCase):
    ...
    def test_cgroupv1(self):
        logging.getLogger("nsbox.nsjail").setLevel(logging.ERROR)
        logging.getLogger("nsbox.utils.swap").setLevel(logging.ERROR)
