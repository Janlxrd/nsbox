...
# Everything will be a user install to allow nsbox's dependencies to be kept
...
COPY requirements/ /nsbox/requirements/
WORKDIR /nsbox
...
RUN if [ -n "${DEV}" ]; \
    then \
        ...
        && PYTHONUSERBASE=/nsbox/user_base pip install numpy~=1.19; \
    fi

# At the end to avoid re-installing dependencies when only a config changes.
COPY config/ /nsbox/config/
...
# The venv already installed dependencies, so nothing besides nsbox itself
...
RUN --mount=source=.,target=/nsbox_src,rw \
    pip install /nsbox_src[gunicorn,sentry]